    ...
    var that = {
      ...
      redraw:function(){
        ...
        particleSystem.eachEdge(function(edge, p1, p2){
          ...
          gfx.line(p1, p2, {stroke:"white", width:2, alpha:edge.target.data.alpha})
        })
        ...
        particleSystem.eachNode(function(node, pt){
          ...
          if (node.data.shape=='dot'){
            var w = 75;
            gfx.oval(pt.x-w/2, pt.y-w/2, w, w, {stroke:"white", width:2, fill:node.data.color, alpha:node.data.alpha})
            gfx.text(node.name, pt.x, pt.y+7, {color:"gray", align:"center", font:"Arial", size:12})
            gfx.text(node.name, pt.x, pt.y+7, {color:"gray", align:"center", font:"Arial", size:12})
          }else{
            var w = Math.max(20, 40+gfx.textWidth(node.name) )
            gfx.rect((pt.x-w/2)-20, pt.y-10, w+40, 16+20, 4, {fill:node.data.color, alpha:node.data.alpha})
            gfx.text(node.name, pt.x, pt.y+16, {color:"black", align:"center", font:"Arial", size:16})
            gfx.text(node.name, pt.x, pt.y+16, {color:"black", align:"center", font:"Arial", size:16})
          }
      ...
      initMouseHandling:function(){
        ...
        var handler = {
          ...
          doubleclicked:function(e){
            ...
            $(canvas).bind('dblclick', handler.doubleclicked)
